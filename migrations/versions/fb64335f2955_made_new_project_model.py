...
def upgrade():
    ...
    sa.Column('projectStatus', sa.Enum('PENDING', 'APPROVED', 'REJECTED', 'ASSESSMENT', name='status'), nullable=False),
    ...
    sa.Column('category', sa.Enum('A', 'B', 'C', 'D', name='category'), nullable=True),
...
def downgrade():
    ...
    with op.batch_alter_table('project_user', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key('project_user_projectID_fkey', 'projects', ['projectID'], ['projectID'])

    with op.batch_alter_table('project_task', schema=None) as batch_op:
        batch_op.drop_constraint(None, type_='foreignkey')
        batch_op.create_foreign_key('project_task_projectID_fkey', 'projects', ['projectID'], ['projectID'])

    with op.batch_alter_table('project_status_data', schema=None) as batch_op:
        batch_op.drop_constraint(None, type_='foreignkey')
        batch_op.create_foreign_key('project_status_data_projectID_fkey', 'projects', ['projectID'], ['projectID'])

    with op.batch_alter_table('project_stage', schema=None) as batch_op:
        batch_op.drop_constraint(None, type_='foreignkey')
        batch_op.create_foreign_key('project_stage_projectID_fkey', 'projects', ['projectID'], ['projectID'])
    ...
    with op.batch_alter_table('assessment_answers', schema=None) as batch_op:
        batch_op.drop_constraint(None, type_='foreignkey')
        batch_op.create_foreign_key('assessment_answers_projectID_fkey', 'projects', ['projectID'], ['projectID'])

    op.create_table('answers',
    sa.Column('answerID', sa.INTEGER(), server_default=sa.text('nextval(\'"answers_answerID_seq"\'::regclass)'), autoincrement=True, nullable=False),
    ...
    sa.ForeignKeyConstraint(['choiceID'], ['question_choices.choiceID'], name='answers_choiceID_fkey'),
    sa.ForeignKeyConstraint(['projectID'], ['projects.projectID'], name='answers_projectID_fkey'),
    sa.ForeignKeyConstraint(['questionID'], ['questions.questionID'], name='answers_questionID_fkey'),
    ...
    sa.Column('budgetRequired', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False),
    sa.Column('budgetAvailable', sa.DOUBLE_PRECISION(precision=53), autoincrement=False, nullable=False),
    sa.Column('projectStatus', postgresql.ENUM('PENDING', 'APPROVED', 'REJECTED', 'pending assessment', 'ASSESSMENT', name='projectstatus'), autoincrement=False, nullable=False),
    sa.Column('projectScope', sa.VARCHAR(), autoincrement=False, nullable=True),
    sa.Column('category', postgresql.ENUM('A', 'B', 'C', 'D', name='projectcategory'), autoincrement=False, nullable=True),
    ...
    sa.ForeignKeyConstraint(['regionID'], ['regions.regionID'], name='projects_regionID_fkey'),
    sa.ForeignKeyConstraint(['userID'], ['users.userID'], name='projects_userID_fkey'),
