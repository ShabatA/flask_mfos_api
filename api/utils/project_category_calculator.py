...
class ProjectCategoryCalculator:
    ...
    def calculateCategory(self):
        ...
        for answer in self.assessment_answers:
            ...
            for category_dict in [budget_dict, project_priority_dict, importance_dict, implementation_dict, marketing_dict]:
                for section, options in category_dict.items():
                    # Convert each option key to lowercase for case-insensitive matching
                    options_lower = {key.lower(): value for key, value in options.items()}
        ...
        points = importance_dict['beneficiaryCategory'].get(str(self.project.beneficiaryCategory), 0)
        total_points += points
        points = project_priority_dict['projectNature'].get(str(self.project.projectNature), 0)
        total_points += points
        points = project_priority_dict['projectScope'].get(str(self.project.projectScope), 0)
        total_points += points
